...
const {Payment, PaymentStatus} = require('./Payment.js');
...
const api = {Payment, PaymentStatus, order, db, Errors};
/**
...
api.createPayment = async ({creator, paymentData}) => {
  const {service, amount, orders} = paymentData;
  const paymentId = `urn:uuid:${bedrock.util.uuid()}`;
...
api.updatePendingPayment = async ({pendingPayments, paymentData}) => {
  ...
  logger.debug('Pending Payments', {pendingPayments, payment});
  ...
  logger.debug('Pending Payments', {updatedOrder, payment});
  await db.save({payment});
